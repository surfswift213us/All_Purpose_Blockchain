...
#if defined(NDEBUG)
# error "AllPurposeCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x35227e997952777e81d4c3642900233b18598527f9cfad615e404077238e661b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // AllPurposeCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "AllPurposeCoin Signed Message:\n";
...
{
    // AllPurposeCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // AllPurposeCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // AllPurposeCoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // AllPurposeCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // AllPurposeCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // AllPurposeCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xcb343b9c1fb7e69d43e653e1f9e9fa59eaac9389598d99fd04e6f5d6dc368689");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "AUG 08 2018 Richard Careaga Build this Blockchain for APC";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0445a737d6903f4cf0fa01f3a92b63d701385200e39a04c486432fda39a651a0310a190ff3f7a4adfb714847db4154e126568ea8225c8c371c6c7b7c4d96c8e64b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1533745811;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085305186;

        if (fTestNet)
        {
            block.nTime    = 1533745811;
            block.nNonce   = 386778864;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x34bb16520f9606651405be16ac1d3d6c41bb81037aa61cbb36b887fe4bda81b6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x1c, 0xc1, 0xc1, 0x1c }; // AllPurposeCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AllPurposeCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AllPurposeCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AllPurposeCoinMiner(CWallet *pwallet)
{
    printf("AllPurposeCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("allpurposecoin-miner");
    ...
    try { loop {
        ...
        printf("Running AllPurposeCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AllPurposeCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AllPurposeCoinMiner, pwallet));
}
